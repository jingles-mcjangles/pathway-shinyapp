...
dash.sidebar <- dashboardSidebar(
  ...
  sidebarMenu(
    id = "sidebarmenu", startExpanded = T,
    menuItem("Home", tabName = "main", icon = icon("home")),
    fileInput("file1", "Upload CSV File",
      accept = c("text/csv", "text/comma-separated-values,text/plain", ".csv")
    ),
    numericInput("fdr_num", "False Discovery Rate", min = 0, max = 1.0, step = 0.01, value = 0.05, width = "95%"),
      ...
      fluidRow(
        tabBox(
          ...
          tabPanel(
            "iPath: Coverage",
            div(
              style = "height:510px; overflow-y: scroll; overflow-x: scroll; font-size:75%",
              htmlOutput("ipath_coverage") %>% withSpinner()
            )
          ),
          tabPanel(
            "iPath: Significant Metabolites",
            div(
              style = "height:510px; overflow-y: scroll; overflow-x: scroll; font-size:75%",
          ...
          tabPanel(
            "Significantly Changing Metabolites",
            div(
              style = "height:510px; overflow-y: scroll; overflow-x: scroll; font-size:75%",
        ...
        box(
          ...
          div(
            style = "height:330px; overflow-y: scroll; overflow-x: scroll",
      ...
      fluidRow(
        tabBox(
          ...
          tabPanel(
            "Input Data",
            div(
              style = "height:330px; width:500px; overflow-y: scroll; overflow-x: scroll; font-size:65%",
